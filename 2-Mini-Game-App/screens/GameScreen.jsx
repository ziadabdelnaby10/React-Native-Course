...
export function GameScreen({userNumber, onGameOver}) {
    ...
    const renderView = (<View style={styles.screen}>
        <Title>Opponent's Guess</Title>
        <NumberContainer>{currentGuess}</NumberContainer>
        <Card>
            <View>
                <InstructionText style={{marginBottom: 8, fontSize: 28}}>Lower or Higher ?</InstructionText>
        ...
        <View style={styles.logs}>
            <FlatList data={previousLogs}
                      ...
                      renderItem={(itemData) => <GuessLogItem roundNumber={previousLogs.length - itemData.index}
                                                              guess={itemData.item}/>}/>
            {/*previousLogs.map((log, index) => (<InstructionText key={index}>{log}</InstructionText>))*/}
        </View>
    </View>);

    const landscapeView = (<View style={styles.screen}>
        <Title>Opponent's Guess</Title>
